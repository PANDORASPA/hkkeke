...
6. 按「建立批量列隊」只建立任務；按「建立並開始」會立即逐個生成。
7. 生成期間可按「暫停」，系統會在目前任務完成後停止。
8. 可按「繼續列隊」恢復，或按「重試全部失敗」把失敗任務放回等待中。
9. 常用設定可保存成「生產配方」，下次直接套用。
10. 可用每日 seed 隨機化組合；同一 seed 可重現同一批組合，不同 seed 會改變場景和造型分配。
11. 完成後按「匯出生產報告」保存任務、成功率、重試次數和錯誤記錄。
