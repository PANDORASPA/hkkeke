...
5. 先看「生產前檢查」，確認場景數、參考素材、任務數、預估時間和風險提示。
6. 看「生產計劃」確認場景覆蓋和前 8 個任務樣本；需要留底可匯出 JSON。
7. 按「建立批量列隊」只建立任務；按「建立並開始」會立即逐個生成。
8. 生成期間可按「暫停」，系統會在目前任務完成後停止。
9. 可按「繼續列隊」恢復，或按「重試全部失敗」把失敗任務放回等待中。
10. 常用設定可保存成「生產配方」，下次直接套用。
11. 可用每日 seed 隨機化組合；同一 seed 可重現同一批組合，不同 seed 會改變場景和造型分配。
12. 完成後按「匯出生產報告」保存任務、成功率、重試次數和錯誤記錄。
...
5. 用「批次報表」查看每一批生成了幾多張、保留幾多、唔要幾多。
6. 可整批保留、整批唔要、查看批次、匯出單一批次或刪除批次本機圖片。
7. 用「品質報表」查看女仔風格、衣服、髮型、髮色、表情、身材和姿勢的保留率。
8. 按「套用高保留組合到生成」會回到 `/generate`，自動填入高保留率設定和批量 prompt。
9. 把高保留率組合保存成下一次「生產配方」，低保留率組合就減少使用。
10. 匯出素材包 JSON 可備份本機歷史、素材、批次和列隊。
